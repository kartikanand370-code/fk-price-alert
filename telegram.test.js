...
test('keeps scan and bank-alert selections per product', () => {
  const user = bot._test.newUser({ id: '1', first_name: 'Test' }, '1');
  user.products = ['PHONE-1', 'PHONE-2', 'PHONE-3'];
  assert.deepEqual(bot._test.selectedScanProducts(user), ['PHONE-1', 'PHONE-2', 'PHONE-3']);
  user.scanProducts = ['PHONE-2'];
  user.bankAlertProducts = ['PHONE-1', 'PHONE-3'];
  assert.deepEqual(bot._test.selectedScanProducts(user), ['PHONE-2']);
  assert.deepEqual(bot._test.selectedBankAlertProducts(user), ['PHONE-1', 'PHONE-3']);
  const rows = bot._test.selectionRows(user, 'scan').flat();
  assert.equal(rows.some(button => button.callback_data === 'scanprod:1' && button.text.startsWith('✅')), true);
});
